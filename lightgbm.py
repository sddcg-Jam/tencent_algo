...
#,                             names=['样本id','广告id','创建时间','素材尺寸','广告行业id' ,'商品类型','商品id','广告账号id','投放时间','人群定向'
#                                                            ,'出价']
...
# label=test_sampledf['曝光次数'].reset_index(drop=True)
# test_sampledf.drop(['曝光次数'],axis=1,inplace=True)
# train_data = lgb.Dataset(test_sampledf, label=label, feature_name=['广告id','创建时间','素材尺寸','广告行业id','商品类型','商品id','广告账号id','投放时间','人群定向',
#                  '出价','星期','曝光日期'], categorical_feature=['广告id','创建时间','素材尺寸','广告行业id','商品类型','商品id','广告账号id','投放时间','人群定向',
#                  '星期','曝光日期'])
...
# 不符合单调性,所以只取出价在第五位的曝光次数,其余的进行变动
...
# 与NN模型再取均值
nn_result= pd.read_csv('E:\\work\\tencent\\algo.qq.com_641013010_testa\\testA\\submission\\0520\\submission-82.7053.csv',sep=',',
                             header=None,names=['样本id','曝光次数'])
B_sample['82.7053']=nn_result['曝光次数']
B_sample['new曝光1']=(B_sample['82.7053']*2+B_sample['new曝光'])/3
...
for i, (fit_idx, val_idx) in enumerate(cv.split(x_train, y_train)):    
    ...
    sub['value'] += model.predict(x_test, num_iteration=model.best_iteration_)
...
# 评估模型
print('The smape score of prediction is:', getSMAPEScore(y_test,sub['value'])) # 计算真实值和预测值之间的均方根误差
